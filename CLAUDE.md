...
> **Ne bu dosya?** elo web `talimatlar/web/dogrula.js` çalıştığında proje veri köküne `README.md` olarak yazılır. Üç dosya aynı içeriktir: `README.md`, `.cursorrules`, `CLAUDE.md`.
...
| Üretim | 2026-04-14T00:24:54.479Z |
| Talimat slug | `web` |
| Tam kural metni | elo web `talimatlar/web/kurallar.md` |
...
- [ ] `page.json` üst seviyede `name`, `path`, `data`, `index`, `desing`, `modulestatus` alanları mevcut
...
## DIL bulguları — genel yorumlama ve düzeltme (dogrula.js çıktısı)

> Bu bölüm **belirli bir projeye özel değildir**; `talimatlar/web/dogrula.js` veya eşdeğer kontrolün ürettiği **DIL** satırlarını her veri kökünde aynı mantıkla okumak ve gidermek içindir.
...
- Otomatik kurallar ve tam denetim mantığı **ELO web** `talimatlar/web/kurallar.md` + `dogrula.js` üzerindedir. Bu dosya **elle kontrol özeti**dir; denetleyici ile çelişki varsa ELO tarafındaki talimat güncellenmelidir. Bu DIL bölümü, raporu okuyan LLM veya geliştirici için **ortak sözlük ve iş akışı** sağlar.
...
- [ ] `text` HTML tipografi: gövdede `<h1>` yok; ilk bölüm başlığı `<h2>` — metnin en üstünde üst seviye olmadan tek başına `<h3>` yok (`kurallar.md` §6.1)
